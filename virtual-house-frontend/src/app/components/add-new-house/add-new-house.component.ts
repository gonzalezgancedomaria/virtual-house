...
export class AddNewHouseComponent {
  ...
  filePics = 'Select pictures';
  message = '';
  currentFile?: File;
  progress = 0;
  fileInfos?: Observable<any>;

  constructor(private uploadService: FileUploadService, private dialog: MatDialog, private snackBar: MatSnackBar) {
  ...
  public save(){
    this.snackBar.open(this.form.get('name').value + ' se ha añadido con éxito.', 'Close', {
      duration: 3000, 
    });
  }
  ...
  selectPics(event: any): void {
    if (event.target.files && event.target.files) {
      this.filePics = '';
      event.target.file.forEach((file) =>
      this.filePics += file.name + ' ');
    } else {
      this.filePics = 'Select pictures';
    }
  }
}
